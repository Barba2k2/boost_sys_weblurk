...
void main() {
  ...
  group('UserRepository', () {
    test('login success returns access token', () async {
      // Arrange
      ...
      expect(restClient.getCallData('POST', '/auth/login'), {
        'nickname': 'testuser',
    ...
    test('confirmLogin returns model with tokens', () async {
      // Arrange
      restClient.setResponse(
        'PATCH', 
        '/auth/confirm', 
    ...
    test('getUserLogged returns user model', () async {
      // Arrange
      restClient.setResponse(
        'GET', 
        '/user/', 
    ...
    test('updateLoginStatus calls correct endpoint', () async {
      // Arrange
      restClient.setResponse(
        'POST', 
      ...
      expect(restClient.getCallData('POST', '/streamer/status/update'), {
        'streamerId': 1,
        'status': 'ON',
      });
    });
