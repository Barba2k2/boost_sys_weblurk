...
class HomeRepositoryImpl implements HomeRepository {
  HomeRepositoryImpl({
    ...
    required PollingDataSource pollingDataSource,
  ...
  final PollingDataSource _pollingDataSource;
  ...
  Future<AppResult<List<dynamic>>> fetchSchedules() async {
    try {
      ...
      _logger
          .info('Repository: Schedules buscados com sucesso: ${data.length}');
  ...
  Future<AppResult<List<ScheduleListEntity>>> fetchScheduleLists() async {
    try {
      ...
      _logger.info(
          'Repository: Listas de schedules buscadas com sucesso: ${data.length}');
      return AppSuccess(data);
    } catch (e, s) {
      _logger.error(
          'Repository: Erro inesperado ao buscar listas de schedules', e, s);
  ...
  Future<AppResult<List<String>>> getAvailableListNames() async {
    try {
      ...
      _logger.info(
          'Repository: Nomes das listas buscados com sucesso: ${data.length}');
      return AppSuccess(data);
    } catch (e, s) {
      _logger.error(
          'Repository: Erro inesperado ao buscar nomes das listas', e, s);
  ...
  Future<AppResult<ScheduleListEntity?>> fetchScheduleListByName(
    ...
    try {
      ...
      _logger.info(
          'Repository: Lista buscada com sucesso: ${data?.name ?? 'não encontrada'}');
      return AppSuccess(data);
    } catch (e, s) {
      _logger.error(
          'Repository: Erro inesperado ao buscar lista por nome', e, s);
      return AppFailure(Exception('Erro ao buscar lista por nome: $e'));
    }
  }

  @override
  Future<AppResult<AppUnit>> updateLists() async {
    try {
  ...
  Future<AppResult<String?>> fetchCurrentChannelForList(String listName) async {
    try {
      _logger.info(
          'Repository: Iniciando busca de canal atual para lista: $listName');
      final data = await _homeService.fetchCurrentChannelForList(listName);
      _logger.info(
          'Repository: Canal atual para lista buscado com sucesso: $data');
      return AppSuccess(data);
    } catch (e, s) {
      _logger.error(
          'Repository: Erro inesperado ao buscar canal atual para lista', e, s);
  ...
  Future<AppResult<AppUnit>> startPolling(int streamerId) async {
    try {
      ...
      await _pollingDataSource.startPolling(streamerId);
  ...
  Future<AppResult<AppUnit>> stopPolling() async {
    try {
      ...
      await _pollingDataSource.stopPolling();
  ...
  @override
  Future<AppResult<AppUnit>> reloadWebView() async {
    try {
